...
class FlashingState4 extends MusicBeatState
{
	...
	override function create()
	{
		...
		warnText = new FlxText(0, 0, FlxG.width,
			"last warning\n\n
			this mod has shaders that can slow down your PC\n
			to deactivate it, go to options, then \"Others\" and deactivate it",
			32);
	...
	override function update(elapsed:Float)
	{
		if(!leftState) {
			...
			if (controls.ACCEPT) {
				...
				FlxTween.tween(tvbords, {alpha:0}, 1, {ease: FlxEase.sineIn});
				if(!back) {
